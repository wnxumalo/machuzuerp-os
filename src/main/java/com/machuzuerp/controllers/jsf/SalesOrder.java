...
import com.machuzuerp.jdbc.SalesOrderProcessing;
...
public class SalesOrder implements Serializable {
    ...
    SalesOrderProcessing qp = new SalesOrderProcessing();
    ...
    private List<ServiceData> selectedServs = new ArrayList<ServiceData>();
    private List<InventoryData> selectedInvs = new ArrayList<InventoryData>();
    ...
    public String saveSalesOrder() {
        ...
        try {
            ...
            qp.editSalesOrder(request.getSession().getAttribute("uk").toString(), "", "", "");
            ret = "/payable/salesorder/saved";
        } catch (Exception npe) {
            ret = "/payable/salesorder/notsaved";
        }

        return ret;
    }

    public String saveEditSalesOrder() throws SQLException {
        ...
        try {
            ...
            qp.editSalesOrder(request.getSession().getAttribute("uk").toString(), "edit", "", recnum);
            ret = "/payable/salesorder/saved";
    ...
    public String mainEditSalesOrder() throws SQLException {
        getParams();
    ...
    public String editSalesOrder_1(String iRecNum, String iCheck, java.util.Date iDate, String cName, String cSurname, String tel, String cell, String fax, String posAdd, String physAdd, String country, String notes, String cid) throws SQLException, ParseException {
        ...
        return "/payable/salesorder/edit-sales-order";
    }

    public void getSalesOrder() throws SQLException, ParseException {

        qp.getSalesOrder(recnum);
    ...
    public String deleteSalesOrder() throws SQLException {
        getParams();

        try {
            request = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
            qp.editSalesOrder(request.getSession().getAttribute("uk").toString(), "delete", "yes", recnum);
            ret = "/payable/salesorder/saved";
        } catch (NullPointerException npe) {
            ret = "/payable/salesorder/notsaved";
        }
    ...
    public String selectService(String recnum) throws SQLException {
        ...
        return "/accounting/receivable/salesorder/select-service";
    }
    ...
    public String selectInventory() {
        ...
        return "/accounting/receivable/salesorder/select-inventory";
    }
    ...
    public String invSummary(List<ServiceData> selectedServs) {
        ...
        return "/accounting/receivable/salesorder/sales-order-summary";
    }

    public String invComplete(List<ServiceData> selectedServs, List<InventoryData> selectedInvs, SalesOrder salesOrder) throws IOException, SQLException {
        getAccParams();
        ...
        try {
            ...
            String maxRec = qp.saveSalesOrder(invDate, cID, cName, cSurname, tel, cell, fax, posAdd, physAdd, notes, request.getSession().getAttribute("uk").toString());
            iRecNum = maxRec;
            ...
            for (int x = 0; x < count; x++) {
                ...
                this.totServCost = this.totServCost.add(cost.multiply(BigDecimal.valueOf(quantity)));
                selectedServs.get(x).setTotCost(cost.multiply(BigDecimal.valueOf(quantity)));
                qp.saveSalesOrderServices(request.getSession().getAttribute("uk").toString(), maxRec, selectedServs.get(x).getDescription(), selectedServs.get(x).getCost(), selectedServs.get(x).getQty(), (selectedServs.get(x).getCost().multiply(BigDecimal.valueOf(selectedServs.get(x).getQty()))), selectedServs.get(x).getRecNum());
            }
            ...
            for (int x = 0; x < count; x++) {
                ...
                this.totInvCost = this.totInvCost.add(invCost.multiply(BigDecimal.valueOf(quantity)));
                selectedInvs.get(x).setTotCost(invCost.multiply(BigDecimal.valueOf(quantity)));
                qp.saveSalesOrderInventory(request.getSession().getAttribute("uk").toString(), maxRec, selectedInvs.get(x).getDescription(), selectedInvs.get(x).getCost(), selectedInvs.get(x).getQuantity(), (selectedInvs.get(x).getSellingPrice().multiply(BigDecimal.valueOf(selectedInvs.get(x).getQuantity()))), selectedInvs.get(x).getRecNum());
            }

            this.totCost = totInvCost.add(totServCost);
            
            ret = "/accounting/receivable/salesorder/sales-order-complete";
            
        } catch (SQLException e) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error Occurred", "Kindly Check Data Formatting. Replace Single Quotes With Double Quotes"));
            ret = "";
        }
    ...
    public String findSalesOrder(HttpSession session) {
        getParams();
        
        session.setAttribute("salesOrderData", null);
        
        return "/accounting/payable/salesorder/find-sales-order";
    }

    public String printSalesOrder() {
        getParams();
        return "/accounting/payable/salesorder/print-sales-order";
    }
    ...
    public void printPDF() throws IOException {
        ...
        FacesContext.getCurrentInstance().getExternalContext().redirect("print-sales-order.jsp?cName=" + cName + "&cSurname=" + cSurname + "&cell=" + cell + "&tel=" + tel + "&posAdd=" + posAdd + "&servDesc=" + servDesc + "&invDesc=" + invDesc + "&sCost=" + sCost + "&iCost=" + iCost);
